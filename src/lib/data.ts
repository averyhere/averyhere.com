import { AccordionItemType } from "@/components/ui";
...
const work_experience = [
  ...
  },
  {
    ...
    end_date: "May, 2014",
    overview:
      "<p>Designed and developed custom websites for several small businesses along the east coast. Worked with key stakeholders to establish timelines, milestones, and objectives. Designed, built, deployed, and maintained high-performing static sites with integrations to tools such as Google Drive to enable non-tech-savvy business owners to manage content easily.</p>",
  },
];
...
const projects = [
  {
    ...
    overview:
      "<p>The Bald Yeti theme is purpose-built for Headless WordPress websites. It is intended to be used in conjunction with a separate front-end application and does not contain any front-end or public facing assets.</p>",
  ...
  },
  {
    ...
    overview:
      "<p>This WordPress plugin enabled our teams to quickly and easily embed Marketo Forms directly into WordPress websites via a custom Gutenberg block, as well as a custom field added to ACF (Advanced Custom Fields plugin).</p>\n<p>The plugin accepted Marketo REST API credentials, stored them securely, and cached form data to reduce API calls made to Marketo</p>\n<p>Users were presented with a dropdown field to select which of the approved forms in Marketo was needed. Users could also choose a followup action - either defaulting what is configured in the Marketo form, showing a followup message, or redirecting the user to another URL.</p>\n<p>The plugin also output the Marketo Form without including Marketo's heavy MktoForms2.js javascript library on load. Instead, forms were generated using VueJS and submitted securely through the lazy-loaded javascript library. This enabled a better UX for site visitors, faster load times, and a reduction to Page Speed Insights Content Layout Shift scores as forms are rendered immediately. All while retaining and utilizing the very same secure dataflow utilized in native embedded Marketo Forms.</p>",
    link_text: "Check it out!",
